...
class VoiceGateway extends Utils.EventEmitter
{
	...
	{
		this.token = token;
		if (this.sessionId && !this.identified) {
			this.resolvePromises();
			this.connect();
		}
	}
	...
	ssrcToUserId(ssrc, type)
	{
		...
		for (let [userId, userSSRC] of this.ssrcs[type].entries()) {
			if (ssrc === userSSRC) {
				return userId;
			}
		}
	...
	send(op, d, cb, direct)
	{
		...
		if (direct) {
			...
			const func = () => {
				if (this.bucket.locked || !this.ready || !this.connected) {
	...
	identify()
	{
		this.send(OpCodes.IDENTIFY, {
			...
			token: this.token,
			video: !!Constants.Voice.Codecs.VIDEO.length
		}, undefined, true);
	}
	...
	handle(data)
	{
		...
		switch(packet.op) {
			...
			case OpCodes.SPEAKING: {
				this.ssrcs.audio.set(packet.d.user_id, packet.d.ssrc);
			...
			case OpCodes.CLIENT_CONNECT: {
				this.ssrcs.audio.set(packet.d.user_id, packet.d.audio_ssrc);
				if (packet.d.video_ssrc) {
					this.ssrcs.video.set(packet.d.user_id, packet.d.video_ssrc);
				}
				//start the user id's decode/encoders too
			}; break;
			case OpCodes.CLIENT_DISCONNECT: {
				this.ssrcs.audio.delete(packet.d.user_id);
				this.ssrcs.video.delete(packet.d.user_id);
			}; break;
			case OpCodes.SIGNAL: {
			...
			case OpCodes.CODECS: {
				this.setAudioCodec(packet.d.audio_codec);
				this.setVideoCodec(packet.d.video_codec);
				this.udp.setTransportId(packet.d.media_session_id);
			}; break;
		}
	}
	...
	kill()
	{
		...
		this.resolvePromises(new Error('Voice Gateway was killed.'));
	}

	udpConnect(udpinfo)
	{
		this.ssrcs.audio.set(this.gateway.userId, udpinfo.ssrc);
		...
		if (!udpinfo.mode) {
			this.udp.disconnect();
			this.udp = null;
			return this.emit('error', new Error(`No supported voice mode found in ${JSON.stringify(udpinfo.modes)}`));
		}
	...
	sendSelectProtocol(protocol, data, codecs, sdp, cb)
	{
		this.send(OpCodes.SELECT_PROTOCOL, {protocol, data, codecs, sdp}, cb);
	}

	sendSpeaking(speaking, delay=0, cb)
	{
	...
	sendClientConnect()
	{
		...
		this.send(OpCodes.CLIENT_CONNECT, {
			audio_ssrc: this.udp.ssrc,
			video_ssrc: 0,
			rtx_ssrc: 0
		});
	}

	sendSignal(userId, options)
	{
		//??
