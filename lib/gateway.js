		...
		switch(packet.op) {
			...
			case OpCodes.DISPATCH: {
				if (packet.t === 'READY') {
					...
					this.discordTrace = packet.d._trace;
					this.emit('ready');
					...
					if (packet.d.user_id === this.userId) {
						const serverId = packet.d.guild_id || packet.d.channel_id;
						if (!this.voiceGateways.has(serverId)) {return;}
						const gateway = this.voiceGateways.get(serverId);
						if (!packet.d.channel_id) {
							gateway.kill();
						} else {
							gateway.setChannelId(packet.d.channel_id);
							if (gateway.sessionId !== packet.d.session_id) {
	...
	voiceConnect(guildId, channelId, options)
	{
		...
		return new Promise((resolve, reject) => {
			...
			if (this.voiceGateways.has(serverId)) {
				...
				if (!channelId) {
					return this.voiceStateUpdate(guildId, channelId, options);
				}
