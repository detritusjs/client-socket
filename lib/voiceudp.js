...
const RTCPPacketTypes = Object.values(Packet.RTCPHeader.PacketTypes);
const RTPPayloadTypes = Object.values(Packet.RTPHeader.PayloadTypes);
...
class VoiceUDP extends Utils.EventEmitter {
	constructor(voiceGateway) {
		...
		Object.defineProperties(this, {
			...
			caches: {value: {audio: Buffer.alloc(5 * 1024)}}
		});

		if (this.videoEnabled) {
			this.headers.video = new Voice.RTPHeader({randomize: true});
			this.nonces.video = new Voice.RTPNonce({randomize: true});
	...
	get audioSSRC() {return this.voiceGateway.audioSSRC;}
	get videoSSRC() {return this.voiceGateway.videoSSRC;}
	get rtxSSRC() {return this.voiceGateway.rtxSSRC;}

	get videoEnabled() {
		return this.voiceGateway.videoEnabled;
	}
	...
	get videoPayloadType() {
		return (this.codecs.video) ? Packet.RTPHeader.PayloadTypes[this.codecs.video.toUpperCase()] : null;
	}

	get rtxPayloadType() {
		const payloadType = this.videoPayloadType;
		return (payloadType !== null) ? payloadType + 1 : null;
	}
	...
	setVideoCodec(codec) {
		...
		if (!Codecs.VIDEO.includes(codec)) {
			this.emit('warn', new Error(`Unsupported video codec received: ${codec}, supported; ${JSON.stringify(Codecs.VIDEO)}`));
			return this.voiceGateway.kill();
		}

		this.codecs.video = codec;
		if (this.headers.video) {
			this.headers.video.setPayloadType(this.videoPayloadType);
		}
	}
	...
	connect(ip, port) {
		...
		socket.once('message', (packet) => {
			...
			Codecs.VIDEO.forEach((codec, i) => {
				const payloadType = Packet.RTPHeader.PayloadTypes[codec.toUpperCase()];
				codecs.push({
					name: codec,
					type: 'video',
					priority: (i + 1) * 1000,
					payload_type: payloadType,
					rtx_payload_type: payloadType + 1
				});
			});
	...
	onPacket(packet, rinfo) {
		...
		if (!this.key) {
			const error = new Error('Received a packet before the Session Description');
			error.rinfo = rinfo;
			error.packet = packet;
			return this.emit('warn', error);
		}
		...
		if (RTCPPacketTypes.includes(packetType)) {
			const rtcp = Voice.RTCP.create(packet);
			const format = 'rtcp';
			//rtcp stuff
			//discord uses SR, RR, RTPFB, PSFB
			...
			const userId = (ssrc) ? this.voiceGateway.ssrcToUserId(ssrc, 'audio') : null;
			this.emit('packet', {rinfo, rtcp, format, userId});
		} else {
			const rtp = {
				header: new Voice.RTPHeader({buffer: packet.slice(0, 12)})
			};
			...
			if (!RTPPayloadTypes.includes(payloadType)) {
			...
			switch (payloadType) {
				...
				case Packet.RTPHeader.PayloadTypes.VP8: {
					payloadType = 'VP8';
					format = 'video';
				}; break;
				case Packet.RTPHeader.PayloadTypes.VP9: {
					payloadType = 'VP9';
					format = 'video';
				}; break;
				case Packet.RTPHeader.PayloadTypes.H264: {
					payloadType = 'H264';
					format = 'video';
				}; break;
				default: {
			...
			//if video isnt enabled and format is video, drop packet
			if (format === 'video' && !this.voiceGateway.videoEnabled) {
				return;
			}
			...
			if (payloadType === 'VP8') {
				data = new Voice.VP8Payload(data);
			}

			const userId = (format) ? this.voiceGateway.ssrcToUserId(rtp.header.ssrc, format) : null;
			this.emit('packet', {rinfo, rtp, payloadType, format, isRTX, userId, data});
		}
	}
	...
	sendAudioFrame(packet, options) {
		this.sendFrame(packet, Object.assign({}, options, {type: 'audio'}));
	}

	sendVideoFrame(packet, options) {
		this.sendFrame(packet, Object.assign({}, options, {type: 'video'}));
	}
	...
	sendFrame(packet, options) {
		...
		if (type === 'video' && !this.videoEnabled) {
			throw new Error('Cannot send in video frames when video is disabled!');
		}

		const rtp = {};
		if (options.useCache) {
			rtp.header = this.headers[type];
			rtp.nonce = this.nonces[type];
		} else {
			...
			switch (type) {
				...
				case 'video': {
					payloadType = this.videoPayloadType;
					ssrc = this.videoSSRC;
				}; break;
				default: throw new Error(`Invalid type ${type}`);
			}
			rtp.header = new Voice.RTPHeader({payloadType, ssrc});
			rtp.nonce = new Voice.RTPNonce({randomize: true});
		}

		if (!options.useCache) {
			if (options.sequence === undefined) {
				options.sequence = this.headers[type].sequence;
				options.incrementSequence = false;
			}
			if (options.timestamp === undefined) {
				options.timestamp = this.headers[type].timestamp;
		...
		const cache = (options.useCache) ? this.caches[type].slice(12) : null;
		...
		if (options.useCache) {
			...
			rtp.header.copy(this.caches[type]);
			data.forEach((buffer) => {
				const start = total;
				total += buffer.length;
				if (buffer.packet) {return;}
				buffer.copy(this.caches[type], start);
			});
			
			buffer = this.caches[type].slice(0, total);
		} else {
			buffer = Buffer.concat([rtp.header.buffer].concat(data).map((b) => (b.packet) ? b.packet : b));
		}
